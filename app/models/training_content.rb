class TrainingContent < ApplicationRecord
  ...
  CONTENT_TYPES = %w[text video audio quiz slides document].freeze
  ...
  def validate_content_data_format
    ...
    when 'quiz'
      unless content_data['questions'].is_a?(Array) && content_data['questions'].all? do |q|
        q['question'].present? && q['answers'].present?
      end
        errors.add(:content_data, 'must contain valid questions array')
      end
    when 'video', 'audio', 'document'
      errors.add(:content_data, 'must contain media URL') unless content_data['url'].present?
    end
  end

  def validate_completion_criteria_format
    case content_type
    when 'video', 'audio', 'text', 'document'
