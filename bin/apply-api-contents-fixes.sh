...
# Script to apply API controller and route fixes for Training Contents
...
echo -e "${YELLOW}Applying API controller and route fixes for Training Contents...${NC}"
echo "==============================================="

# Create backup directory
BACKUP_DIR="api-contents-fixes-backup-$(date +%Y%m%d%H%M%S)"
mkdir -p $BACKUP_DIR

# Backup original files
echo -e "${YELLOW}Backing up original files...${NC}"
cp config/routes/api/v1.rb $BACKUP_DIR/
cp app/controllers/api/v1/training_contents_controller.rb $BACKUP_DIR/

# Apply route fixes
echo -e "${YELLOW}Applying route fixes...${NC}"
cp config/routes/api/v1_contents_fixed.rb config/routes/api/v1.rb

# Apply controller fixes
echo -e "${YELLOW}Applying controller fixes...${NC}"
cp app/controllers/api/v1/training_contents_controller_fixed.rb app/controllers/api/v1/training_contents_controller.rb
...
echo -e "${GREEN}✓${NC} Applied fixes to TrainingContentsController"
echo -e "${YELLOW}!${NC} Check test results above to see if fixes were successful"
echo "==============================================="
echo -e "${GREEN}RESULT: API_CONTENTS_FIXES_APPLIED${NC}"
...
echo "cp $BACKUP_DIR/training_contents_controller.rb app/controllers/api/v1/training_contents_controller.rb"
